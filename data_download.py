...
BATCH_SIZE = 100                 # how many per batch (when batching)
...
# New toggles
SINGLE_FILE_MODE = False           # if True, disable batching and save one Excel
...
for i, sample in iter_with_index(subset, start=1):
    ...
    # Debug: print current index, batch_num, next_cutoff, window size
    # print(f"[DEBUG] i={i}, batch_num={batch_num}, next_cutoff={next_cutoff}, current_window={len(current_window)}")

    # Decide window boundary:
    boundary = (i >= next_cutoff)
    if boundary:
        print(f"[DEBUG] Boundary reached at i={i}, batch_num={batch_num}")
        # Parallel execution
        ...
        # Debug: print rows collected for batch
        print(f"[DEBUG] Saving batch_num={batch_num}, rows={len(rows)}")
...
# Process leftovers
if current_window:
    print(f"[DEBUG] Processing leftovers: batch_num={batch_num}, current_window={len(current_window)}")
    with ThreadPoolExecutor(max_workers=WORKERS) as ex:
        futures = {ex.submit(fetch_and_save, item): item[0] for item in current_window}
        for fut in as_completed(futures):
            idx, record, err = fut.result()
    ...
    print(f"[DEBUG] Leftover rows to save: batch_num={batch_num}, rows={len(rows)}")
    df = pd.DataFrame(rows)
    if SINGLE_FILE_MODE:
        out_file = f"{EXCEL_PREFIX}.xlsx"
